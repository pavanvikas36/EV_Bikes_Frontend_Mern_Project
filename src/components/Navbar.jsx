...
function Navbar({ isLoggedIn, userRole }) {
  ...
  return (
    <nav
      className={`bg-black text-white px-4 md:px-6 py-3 sticky top-0 z-50 transition-all ${
        scrolled ? "bg-opacity-95 backdrop-blur-sm shadow-md" : ""
      }`}
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        {/* Logo */}
          ...
          <ul className="hidden md:flex gap-6 font-medium">
            {menuItems.map((item, idx) => (
              <li
                key={idx}
                onClick={() => navigate(item.path)}
                className={`cursor-pointer border-b-2 py-1 transition ${
                  location.pathname === item.path
                    ? "text-orange-500 border-orange-500"
        ...
        {/* Right Section */}
        <div className="flex items-center gap-3 md:gap-4">
          {/* 🔍 Search Bar (ONLY logged-in buyer) */}
          {isLoggedIn && userRole === "buyer" && (
            <form
              onSubmit={handleSearch}
              className="hidden md:flex items-center bg-gray-800 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-orange-500"
            >
              <input
                type="text"
                placeholder="Search vehicles..."
              ...
              <button
                type="submit"
                className="px-2 py-1.5 bg-orange-500 hover:bg-orange-600"
              >
                <Search size={18} />
              </button>
            </form>
          ...
          {isLoggedIn ? (
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-gray-900 rounded-lg shadow-xl border border-gray-700 overflow-hidden">
                  ...
                  <button
                    ...
                    className="flex items-center w-full px-4 py-3 hover:bg-orange-500"
                  >
                    <UserCog size={18} className="mr-2" />
                    Profile
                  </button>
                  ...
                  <button
                    onClick={handleLogout}
                    className="flex items-center w-full px-4 py-3 hover:bg-orange-500 border-t border-gray-700"
                  >
                    <LogOut size={18} className="mr-2" />
                    Logout
                  </button>
            ...
            <button
              onClick={() => navigate("/login")}
              className="bg-orange-500 px-4 py-1.5 rounded-lg hover:bg-orange-600"
            >
              Login
            </button>
          )}

          {/* Mobile Menu Toggle (Buyer only) */}
          {userRole === "buyer" && (
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
      ...
      {isMobileMenuOpen && userRole === "buyer" && (
        <div className="md:hidden bg-gray-900 border-t border-gray-700">
          {/* Mobile Search (ONLY logged-in buyer) */}
          {isLoggedIn && userRole === "buyer" && (
            <form
              onSubmit={handleSearch}
              className="flex p-3 border-b border-gray-700"
            >
              <input
                ...
                className="flex-1 px-3 py-2 bg-gray-800 text-white rounded-l-lg focus:outline-none"
              />
              <button className="px-3 py-2 bg-orange-500 rounded-r-lg">
                <Search size={18} />
              </button>
            </form>
          )}
          ...
          {menuItems.map((item, idx) => (
            <button
              ...
              className={`w-full px-4 py-3 text-left border-b border-gray-800 ${
                location.pathname === item.path
                  ? "bg-orange-500"
                  : "hover:bg-gray-800"
              }`}
